...
const TeamControlPanel: React.FC<TeamControlPanelProps> = ({ team, teamId }) => {
  ...
  const handleScoreChange = (actionValue: number) => {
    ...
    updateScore({ teamId: teamId, action });
  };
  ...
  const handleAddMajorPenalty = () => updatePenalty({ teamId: teamId, type: 'major', action: 1 });
  const handleAddMinorPenalty = () => updatePenalty({ teamId: teamId, type: 'minor', action: 1 });
